...
describe('useFeedbackQueue', () => {
  ...
  it('should peek all queued entries', async () => {
    ...
    await act(async () => {
      await result.current.enqueue({ experienceId: 'track-1', message: 'First' });
    });

    await act(async () => {
      await result.current.enqueue({ experienceId: 'track-2', message: 'Second' });
    });
  ...
  it('should remove a specific entry by id', async () => {
    ...
    let entryId: string | undefined;
    await act(async () => {
      entryId = await result.current.enqueue({ experienceId: 'track-1', message: 'To remove' });
    });
  ...
  it('should clear the entire queue', async () => {
    ...
    await act(async () => {
      await result.current.enqueue({ experienceId: 'track-1', message: 'A' });
    });
    await act(async () => {
      await result.current.enqueue({ experienceId: 'track-2', message: 'B' });
    });
