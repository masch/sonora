...
describe('FeedbackPostBodySchema', () => {
  ...
  describe('tripId', () => {
    ...
    it('rejects empty tripId', () => {
      const result = FeedbackPostBodySchema.safeParse({ ...validPayload, tripId: '' });
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('type coercion', () => {
    it('rejects non-string tripId', () => {
      const result = FeedbackPostBodySchema.safeParse({ ...validPayload, tripId: 123 });
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('error messages', () => {
    it('includes field name in required error', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues.some((i) => i.path.includes('tripId'))).toBe(true);
