...
| **screen-wrapper.tsx** | — _(new file)_                                                    | `ScreenWrapper` + `ScrollScreenWrapper`             | Trivial    | —   |
...
export function ScreenWrapper({ children, className }) {
  return (
    <TwView className={`flex-1 bg-background${className ? ` ${className}` : ''}`}>
      {children}
    </TwView>
  );
}
...
| `screen-wrapper.tsx` | _(new file)_                                                 | `TwView`, `TwScrollView`                    |
