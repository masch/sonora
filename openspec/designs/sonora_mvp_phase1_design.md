...
- **Calculations:** Uses local helper `utils/haversine.ts`. Unlocks target when distance is $\le 150$ meters. Sets `gpsStatus = 'weak'` if coordinate accuracy error is $> 30$ meters.
